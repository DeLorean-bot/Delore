...
> **ВНИМАНИЕ. Разделы 6, 8, 9.3, 10 и 11 ниже устарели.** Они описывают
> состояние до 27 июля 2026. Актуальное состояние — в разделе 19 в конце
> документа. Читайте его первым; старые разделы оставлены как история
> решений, но их указания больше не выполнять.
...
# 19. Состояние на 27 июля 2026 (вторая сессия)

Работа велась в git. Baseline — коммит `5b8173e`, поверх него 15 коммитов.
Любой шаг откатывается точечно: `git log --oneline`, затем
`git revert <hash>`.

## 19.1 Главное, что выяснилось

**На Skia линза преломляет только `LiquidGlassView.backgroundWidget`.**
Всё, что лежит в `child`, рисуется поверх захвата и в него не попадает
никогда. Проверено по `_paintSkiaCapture` в
`render_liquid_glass_lens.dart`: шейдеру отдаётся картинка захвата фона,
и больше ничего.

Из этого следуют три вывода, которые закрывают открытые вопросы старых
разделов:

1. `_SidebarUnderlay` (раздел 9.3) не просто не преломлялся — он был
   **невидим**. Линза рисует непрозрачный сэмпл захвата по всему своему
   прямоугольнику (`honorBackdropAlpha: false`), поэтому любой слой под
   ней в дереве перекрыт. Удалён.
2. Пока страница лежала в `child`, стекло преломляло декоративный фон, а
   не интерфейс. Поэтому оно и выглядело как полупрозрачная панель, а не
   как стекло. Никакая настройка параметров это не исправила бы.
3. Чтобы получить эффект как в Telegram для iPhone, страница должна
   попасть в захват. Это и сделано, см. 19.3.

## 19.2 Фон (бывший этап 3) — сделано

Backdrop рисовался **дважды**: в `backgroundWidget` и ещё раз в теле
scaffold, то есть в `child` поверх захвата. Копия сверху непрозрачная,
поэтому сцену никто никогда не видел, а полноэкранная отрисовка шла два
раза за кадр. Побочно это скрывало пользовательскую фоновую картинку —
она добавляется только в сцену, то есть фича была визуально мертва.

Теперь фон один, в сцене. Новый файл `lib/widgets/routex_backdrop.dart`:
пять мягких mint/blue/amber пятен, детерминированное зерно, виньетка.
Базовую заливку не рисует — градиент, светлая тема и фоновая картинка не
тронуты. На контентных экранах приглушён, под Reduce Motion не двигается.
Playground рендерит **тот же** painter, чтобы настраиваемое и отгружаемое
не разошлись.

`pixelRatio` захвата теперь `devicePixelRatioOf(context).clamp(1, 2)`
вместо жёсткой `1`: на масштабе Windows 125/150% фиксированная единица
была ниже нативной и захват растягивался вверх.

## 19.3 Разделение слоёв в CommonScaffold — сделано

`lib/widgets/scaffold.dart` разложен на два слоя:

- **`backgroundWidget`** — фон плюс сама страница. Это и есть захват,
  поэтому контент, проходящий под шапкой и под нижним баром, — то, что
  стекло преломляет.
- **`child`** — только стеклянная обвязка: шапка, sidebar, нижний бар.

Оба слоя размечаются по одним правилам. Шапка — константа
`_appBarHeight = 64`, её резервирует контентный слой (флаг `showAppBar`
выключает и её, и резерв). Нижний бар и sidebar продублированы в
контентном слое призраками через
`Visibility(visible: false, maintainSize/State/Animation: true)`: они
раскладываются и анимируются в полный размер, но не рисуются, поэтому
контент подрезан ровно там, где стоит обвязка.

Слой обвязки — `Stack` из `Positioned`, поэтому пустая середина не
поглощает попадания и клики уходят в страницу под стеклом. **Проверено
вживую, клики работают.**

Следствие: `blur` навигации пришлось поднять с 3 до 16, панелей до 10.
Пока бары преломляли пустой mesh, низкое значение было незаметно; когда
под стекло поехал реальный текст, преломление тащило по канту читаемую
смазанную строку вместо матовой размывки.

## 19.4 Семантические варианты материала (бывший этап 4) — сделано

`routeXGlassStyle(context, variant, {radius, capsule})` в
`lib/common/premium_theme.dart` — **единственное** место, где строится
`LiquidGlassStyle`. Пять вариантов: `navigation`, `selection`, `panel`,
`dialog`, `control`. Одиннадцать вызовов перешли с сырого `blur: 28` на
имя роли.

Различия обоснованы, а не на глаз: у `dialog` наименьшее смещение и
наибольшая плотность (на нём лежит текст); у `control` и `selection`
узкая полоса преломления 12 и 10 px, иначе внутри цели 44×44 фаски
противоположных краёв встречаются.

## 19.5 Кромка — четыре захода, итог

Это заняло больше всего времени и было исправлено не с первого раза.
Порядок причин, от ложных к настоящей:

1. Пробовал взять официальный тюнинг `nav_bar_tuning.dart` целиком —
   `borderSolidity: 1`, `borderSaturation: 1.2`. **Ошибка.** Он настроен
   под фотографический фон примера, где кант забирает реальный цвет
   из-под стекла. Над почти чёрным он схлопывается в жёсткую бирюзовую
   линию поверх плашки. Откачено.
2. Кольцо по всему периметру рисует **`ambientIntensity`** — он не
   зависит от угла падения света, поэтому любое ненулевое значение
   обводит фигуру целиком. Сейчас `0`. `lightSpread: 0.06` — блик
   остаётся только там, куда падает свет.
3. Ширина: в оптическом режиме полоса канта равна
   `borderWidth * 2 + 2` **логических пикселя**. То есть «0.8» было
   штрихом в 3.6 px. Сейчас 0.3–0.5 по вариантам.
4. **Настоящая причина «рваности» — сглаживание, а не оформление.**
   Skia антиалиасит скруглённый клип на каждой операции рисования
   отдельно, и на кривой это даёт ступеньку. Плюс `clipQuality: exact`
   обводит continuous-угол полилинией из 40 отрезков — фасеточной по
   построению, — а шейдер рисует squircle-SDF внутри круглого клипа.

Итог: **везде обычный круглый угол** (`LiquidGlassShape.roundedRectangle`
+ `clipQuality: roundedRectangle`) — единственная форма, где шейдер, клип
и тень это одна аналитическая кривая. И **каждая** стеклянная поверхность
обёрнута в `ClipRRect(clipBehavior: Clip.antiAliasWithSaveLayer)`, чтобы
граница сглаживалась один раз при композитинге слоя.

Squircle красивее, но на этом рендерере его нельзя нарисовать чисто.
Если понадобится вернуть — сначала патчить пакет.

**Не проверено:** если ступеньки всё ещё видны, остаётся масштаб дисплея.
На Skia-пути шейдеру передаётся `scale: 1.0`, то есть контур считается в
логических пикселях, а рисуется в физические. При 125/150% это даёт
лестницу, и лечится только правкой `liquid_glass_easy`.

## 19.6 Навигация (бывший этап 5) — сделано, с отступлением от плана

**Официальный `LiquidGlassBottomNavBar` сознательно не взят.** На Skia
его выделение — плоская цветная заливка во всех режимах, кроме
преломляющей morph-pill, а та требует отдать всю страницу в его
собственный пайплайн (`buildGlassPillBar(body: ...)`); библиотека прямо
пишет, что без реального `body` захват на Skia приходит чёрным. Цена —
второй полноэкранный захват на кадр плюс потеря уже имеющейся
преломляющей линзы выделения.

Взята физика вместо компонента:
`lib/widgets/routex_jelly_selection.dart` крутит собственную
`RouteXSelectionGlass` через официальный `LiquidGlassJelly` (он
деформирует **изменением размера**, поэтому линза перепреломляет на
деформированных габаритах) плюс `SpringSimulation` для перемещения, на
константах библиотеки. Повторный клик на лету перезапускает пружину из
текущей позиции с текущей скоростью — траектория гнётся, а не начинается
заново. Reduce Motion — мгновенный переход без тикера.

Используется обеими поверхностями: compact-бар (горизонтально) и
desktop-sidebar (вертикально).

Метрики бара выправлены по референсу Telegram для iPhone: иконки 20 → 25,
подписи 9 → 11.5 и белые вместо серых, высота 60 → 68, бар и кнопка Tools
— настоящие капсулы (радиус = высота / 2).

## 19.7 Glass Playground (бывший этап 2) — сделано

`Tools → Developer → RouteX Glass Playground`.
Файлы: `lib/views/dev/glass_params.dart`, `glass_backdrops.dart`,
`glass_playground.dart`.

Слева превью со своим `LiquidGlassView`, справа около сорока живых
регуляторов — вся поверхность `LiquidGlassStyle` плюс параметры уровня
view. Образцы: panel, dialog, control, круглая кнопка и nav-капсула с
**вложенной** selection-линзой. Пресеты: пять отгружаемых вариантов и
официальные для сравнения. Кнопка **Copy Dart** кладёт готовый
`LiquidGlassStyle(...)` в буфер — прямой мост в токен.

Фоны: `current`, `mesh` (тот же painter, что в продукте), и
диагностические `grid`, `spectrum`, `flatBlack`. Переключатель
`local underlay` показывает, что цветной блок внутри
`LiquidGlassView.child` в захват не попадает.

**Внимание:** пресеты в `glass_params.dart` — ручная копия значений из
`premium_theme.dart`. При правке токенов их надо синхронизировать
руками, автоматической связи нет.

## 19.8 Официальный example

Собран и лежит рядом для сравнения на том же Windows-бэкенде:

    Start-Process -FilePath "D:\liquid_glass_easy-main\example\build\windows\x64\runner\Release\liquid_glass_easy_example.exe" -WorkingDirectory "D:\liquid_glass_easy-main\example\build\windows\x64\runner\Release"

Приём: поставить в Playground фон `spectrum` и открыть example рядом. При
одинаково насыщенном фоне видно, отличается ли материал или отличался
только фон.

## 19.9 Что осталось — по приоритету

1. **Производительность не измерена.** Захват теперь включает всю
   страницу, в плотности дисплея, на Dashboard ещё и realtime, плюс
   save layer на каждую стеклянную поверхность. Это заметно дороже, чем
   было. Крутить `pixelRatio` и `refreshRate` в `scaffold.dart`.
2. **Композиция Dashboard.** Шапку-дубликат с него убрали, но осталось:
   карточка висит по центру с пустотой сверху и снизу; внутри три уровня
   вложенных прямоугольников (карточка → блок статистики → кнопки);
   CTA — плоская мятная капсула с треугольником, без подписи и состояния;
   «Осталось дней: 0» выглядит как поломка; полоса прогресса почти пустая
   при 1.5 ТБ из 1.95 ТБ — похоже на неверную привязку.
3. **Стекло на карточках.** Внутренние блоки на Dashboard — обычные
   `Container` с белой заливкой и рамкой, не стекло. Правильный ход не
   «завернуть каждый в линзу» (документация пакета против множества линз
   в списках), а убрать вложенность: положить группы прямо на фон
   отдельными стеклянными панелями.
4. **Остальные экраны** — Applications, Proxies, Profiles, Connections,
   Settings: та же работа, что на Dashboard, плюс единая шкала радиусов и
   типографика.
5. **Светлая тема и desktop-раскладка** после разделения слоёв не
   осматривались.

## 19.10 Известные предупреждения analyze

`flutter analyze lib` даёт около 600 замечаний — это **наследие
FlClashX**, не наши файлы. Все файлы, затронутые в этой сессии, чистые.
Старые предупреждения в `hero_connect.dart` из раздела 15 остались:
неиспользуемый `_LegacyHeroConnect` и два discarded Future.

## 19.11 Чем смотреть

Приложение можно поднять и осматривать через computer-use: `request_access`
на `flclashx.exe`, затем позиционировать окно через `MoveWindow`. Compact
проверялся на 430×920. Ярлыки `RouteX` созданы на рабочем столе и в меню
«Пуск» и ведут в `build\windows\x64\runner\Release`, то есть всегда
открывают последнюю сборку.
