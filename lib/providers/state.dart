...
bool newDashboardEnabled(Ref ref) {
  // The legacy widget grid belongs to the upstream client. The custom shell
  // always uses the new hero dashboard so old FlClashX visuals cannot leak
  // back through a persisted preference or subscription header.
  return true;
}
...
String? backgroundUrl(Ref ref) {
  ...
  return backgroundUrlFromHeader(
      profile?.providerHeaders['flclashx-background']);
}
