...
Future<ImageProvider?>? windowsProcessIcon(String connectionId) {
  ...
  return windowsExecutableIcon(path);
}

/// Icon embedded in a Windows executable. Cached by path so application
/// discovery and the connections page share the same decoded image.
Future<ImageProvider?> windowsExecutableIcon(String executablePath) {
  if (executablePath.isEmpty) return Future.value();
  return _winIconCache.putIfAbsent(
    executablePath,
    () => _loadWindowsIcon(executablePath),
  );
}
