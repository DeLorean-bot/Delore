...
**Hysteria2 на Windows**

* Исправлены массовые ложные `Timeout` у Hysteria2, когда одновременно запущен другой VPN/Clash-клиент
* Delore теперь автоматически привязывает собственные proxy-соединения к реальному Ethernet или Wi‑Fi, не позволяя чужому TUN перехватывать и блокировать UDP/QUIC-трафик
* Удалена устаревшая глобальная подмена TLS fingerprint; индивидуальные настройки серверов сохраняются без изменений
* Проверено на реальной смешанной подписке при одновременно запущенном Koala Clash: рабочие Hysteria2-узлы снова показывают реальную задержку вместо общего `Timeout`

## v0.5.9

**Современные протоколы**

* Исправлена совместимость Hysteria2, Trojan, XHTTP, HTTPUpgrade и VLESS с PQ/ML-KEM: Delore снова использует официальную TLS-реализацию, с которой выпускается Mihomo
* Проверено на реальной смешанной подписке: узлы всех пяти типов устанавливают соединение и корректно проходят проверку задержки

**Проверка задержки**

* Убраны ложные `Timeout` при проверке больших подписок — тесты теперь запускаются небольшими управляемыми пачками
* Кнопка проверки всех групп больше не тестирует один и тот же сервер несколько раз одновременно через разные селекторы
* Увеличено время первого соединения для QUIC, Reality, ML-KEM и XHTTP: холодный TLS/QUIC handshake больше не обрывается раньше времени
* Исправлен разрыв между таймаутом интерфейса и ядра, из-за которого Delore мог показать `Timeout`, пока Mihomo ещё завершал успешное соединение

## v0.5.8

**Маршрутизация сайтов**

* Настройки сайтов теперь находятся прямо внутри раздела Browser — отдельная вкладка больше не нужна
* Добавлена боковая панель со всеми сохранёнными доменами и выбранными для них локациями
* Флаг выбранной страны теперь виден рядом с локацией как у открытой вкладки, так и в списке настроенных сайтов
* Маршрут сайта сохраняется после обновления вкладки, перезапуска браузера и самого Delore
* Правила отдельных сайтов теперь имеют приоритет над общим маршрутом браузера, включая глобальный режим
* При смене локации активные соединения обновляются сразу, поэтому браузер больше не продолжает использовать старый маршрут
* Правило основного домена автоматически применяется к его поддоменам, при этом более точное правило всегда имеет приоритет
* Delore предупреждает, если маршрут сохранён, но подключение ещё не запущено

**Browser**

* Удалён устаревший отдельный экран настройки сайтов
* Улучшено отображение сохранённых маршрутов в полноэкранном и компактном режимах
* Добавлены проверки, защищающие Browser от пустого экрана и повторного появления отдельной вкладки Sites

## v0.5.7

**Надёжная маршрутизация**

* Исправлена важная разница с Koala Clash: Delore больше не перезаписывает DNS, `strict-route`, определение интерфейса и другие TUN-настройки подписки, если пользователь сам не включил их переопределение
* Добавлена полноценная многолокационная маршрутизация браузера: основной браузер может работать через одну страну, а каждый сайт — через свою
* Правила конкретных сайтов теперь всегда проверяются раньше общего маршрута браузера: например, Chrome → Индия, `youtube.com` → Нидерланды, `github.com` → Германия
* Изменение маршрута одного сайта больше не затрагивает остальные сохранённые сайты

**Browser Bridge**

* Исправлен полностью пустой экран Browser: причиной была некорректная бесконечная высота стеклянной панели
* Добавлены понятные состояния подключения, синхронизации, переподключения и отсутствия обычных вкладок
* Расширение автоматически восстанавливает pairing после обновления или перезапуска Delore
* Синхронизация запускается сразу при пробуждении расширения, поэтому вкладки больше не зависят только от события установки или запуска браузера
* Добавлен безопасный локальный статус Browser Bridge без раскрытия токенов

**Обновления Delore**

* Windows-обновление теперь заранее скачивает установщик, проверяет его SHA-256 и предлагает установить готовое обновление
* Установка проходит автоматически с перезапуском Delore и удалением оставшихся ярлыков старого FlClashX
* Проверка обновлений при запуске включена по умолчанию и использует только репозиторий Delore

**Сообщить о проблеме**

* В Настройки добавлена форма сообщения о проблеме в общем стиле Delore
* Форма собирает шаги воспроизведения, ожидаемый и фактический результат, частоту проблемы и необязательные безопасные диагностические данные
* Можно отправить отчёт анонимно либо оставить контакт; Delore подготовит заполненную GitHub Issue без передачи приватных данных в сторонний сервис
